...
export function getStyles(state: State): Styles {
  ...
  const sheetBorderColor =
    state === State.Onboarding ? "border-[#E5E7EB]" : "border-[#2A42D9]";
  const sheetIconColor =
    state === State.Onboarding ? "text-black" : "text-white";
  const sheetItemsColor =
    state === State.Onboarding ? "text-[#4B5563]" : "text-white";
